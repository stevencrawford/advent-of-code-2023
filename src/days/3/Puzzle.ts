...
type IndexedAny = { value: number | Symbol } & Indexed
type IndexedNumber = { value: number } & Indexed
type IndexedSymbol = { value: Symbol } & Indexed
...
const extractValues = (text: string, pattern: RegExp, buffer: number, processor: Processor<number | Symbol>): IndexedAny[] | IndexedSymbol[] => {
  let result: IndexedAny[] = [];
...
const first = (input: string) => {
  ...
  let partNumbers: IndexedNumber[] = []
  indexedLines.forEach((line, index, lines) => {
    let adjacent: IndexedNumber[] = []
